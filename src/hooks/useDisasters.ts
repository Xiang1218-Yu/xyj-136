...
import { ActiveDisaster, Building, DisasterType } from '../types/game';
...
export function useDisasters({ buildings, onDamageBuildings, onRemoveBuildings }: UseDisastersProps) {
  ...
  const triggerDisaster = useCallback((type: DisasterType) => {
    ...
    const disasterCenter = new THREE.Vector3(x, y, z);
    ...
    buildings.forEach((building) => {
      if (!config.affectedBuildings.includes(building.type)) return;
      ...
      if (distance <= config.damageRadius) {
        ...
        const damage = Math.floor(config.baseDamage * falloff * intensity);
        ...
        if (damage > 0) {
          ...
          if (newHealth <= 0) {
            ...
            damages.push({ id: building.id, damage });
          }
        }
      }
    });
  ...
  useEffect(() => {
    const scheduleNext = () => {
      ...
      timersRef.current.set(`schedule-${type}`, timer);
    };
  ...
  }, [buildings.length, triggerDisaster]);
